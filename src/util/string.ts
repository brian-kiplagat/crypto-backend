...
type DateFormat =
  ...
  | 'DD MMM HH:mm'
  | 'DD-MM-YYYY HH:mm:ss' // 18-03-2025 14:30:00
...
export const formatDate = (
  ...
  return format.replace(
    /YYYY|DD|MMM|MM|MMMM|HH|mm|ss|DDDD/g,
...
export const generateAssetKey = (fileName: string, assetType: string): string => {
  ...
  return `assets/${assetType}s/${uuid}.${extension}`;
};
...
// Utility function to convert minutes to { hours, minutes }
export const minutesToDuration = (minutes: number): { hours: number; minutes: number } => {
  return {
